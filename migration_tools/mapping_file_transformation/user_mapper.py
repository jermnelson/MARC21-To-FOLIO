...
class UserMapper(MappingFileMapperBase):
    def __init__(self, folio_client: FolioClient, use_group_map, args, groups_map=None):
        self.schema = Helper.get_latest_from_github(
            "folio-org", "mod-user-import", "/ramls/schemas/userdataimport.json"
        )
        super().__init__(folio_client, self.schema, None, None)
        if groups_map is None:
            groups_map = []
        self.args = args
        self.use_group_map = use_group_map
        self.groups_map = {}
        self.user_schema = Helper.get_latest_from_github(
            "folio-org", "mod-user-import", "/ramls/schemas/userdataimport.json"
        )
    ...
    def do_map(self, legacy_user, user_map, idx):
                        ...
                        if "properties" in sub_prop:
                            for sub_prop_name2, sub_prop2 in sub_prop[
                                ...
                                if sub_prop2["type"] == "array":
                                    logging.warning("Array: %s", sub_prop_key2)
        ...
        del folio_user["tags"]
        self.report_folio_mapping(folio_user, self.schema)
    ...
    def get_users(self, source_file, file_format: str):
        ...
        for row in reader:
            if len(row.keys()) < 3:
                raise Exception("something is wrong with %s", json.dumps(row))
            yield row
            ...
            if folio_prop_name == "personal.addresses.id":
                ...
                if self.use_group_map:
                    ...
                    if not mapped_legacy_group:
                        logging.fatal(
                            "Patron group %s not in groups map!. Halting", legacy_group
                        )
                        exit()
                    self.migration_report.add(
